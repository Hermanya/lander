...
["actor", "../src/actor.dart", , E, {
  "": "",
  Actor: {
    "": "Object;x>,y>,deltaX,deltaY,deltaVx,deltaVy,acceleration,rotation,fuel,time,isActive,isCrashed",
    Actor$2: function(x, y) {
      this.deltaX = 0.5;
      this.deltaY = 0;
      this.fuel = 100;
      this.time = 0.1;
...
["camera", "../src/camera.dart", , T, {
  "": "",
  Camera: {
    "": "Object;x>,y>,width,height,zoom",
    drawStars$3: function(ctx, stars, actor) {
      var t1, t2, star, t3, t4, t5, t6, t7, t8, t9, t10;
      ctx.beginPath();
      ctx.fillStyle = "#fff";
      for (t1 = new H.ListIterator(stars, stars.length, 0, null); t1.moveNext$0();)
        for (t2 = J.get$iterator$ax(t1._current); t2.moveNext$0();) {
          star = t2.get$current();
          if (star.get$isVisible()) {
            t3 = star.x;
            t4 = star.shiftX;
            if (typeof t3 !== "number")
              return t3.$add();
            ...
            if (typeof t6 !== "number")
              return t6.$div();
            t7 = this.zoom;
            t8 = J.$sub$n(J.$add$ns(star.y, star.shiftY), actor.y);
            t9 = this.height;
            if (typeof t9 !== "number")
              return t9.$div();
            t10 = 1.5 / this.zoom;
            ctx.fillRect(t3 + t4 - t5 + t6 / 2 / t7, J.$add$ns(t8, t9 / 4 / this.zoom), t10, t10);
          }
        }
    },
    drawSurfaceLine$4: function(ctx, surfaceLine, color, actor) {
      var t1, previousPoint, depth, point, t2, t3, t4, t5, t6, t7, isPositiveDiagonal, t8, t9, _color, t10;
      ctx.beginPath();
      ctx.lineWidth = 1 / this.zoom;
      ctx.fillStyle = color;
      ctx.strokeStyle = color;
      t1 = surfaceLine.length;
      if (0 >= t1)
        return H.ioore(surfaceLine, 0);
      previousPoint = surfaceLine[0];
      for (t1 = new H.ListIterator(surfaceLine, t1, 0, null), depth = null; t1.moveNext$0();) {
        point = t1._current;
        ...
        t6 = this.zoom;
        t5 = t5 / 2 / t6;
        if (t3 + 47 > t4 - t5 && t3 - 47 < t4 + t5) {
          t3 = J.getInterceptor$x(previousPoint);
          depth = 0;
          while (true) {
            t4 = actor.y;
            t5 = this.height;
            ...
            if (typeof t7 !== "number")
              return H.iae(t7);
            if (!(depth - 24 < t4 + t5 / 4 * 3 / t6 - t7))
              break;
            isPositiveDiagonal = (C.JSInt_methods.toInt$0(depth) & 1) === 0;
            ctx.beginPath();
            t4 = t3.get$x(previousPoint);
            t5 = actor.x;
            if (typeof t4 !== "number")
              return t4.$sub();
            t6 = this.width;
            if (typeof t6 !== "number")
              return t6.$div();
            t7 = this.zoom;
            t8 = J.$sub$n(t3.get$y(previousPoint), actor.y);
            t9 = this.height;
            if (typeof t9 !== "number")
              return t9.$div();
            ctx.moveTo(t4 - t5 + t6 / 2 / t7, J.$add$ns(J.$add$ns(t8, t9 / 4 / this.zoom), depth));
            t9 = t2.get$x(point);
            t8 = actor.x;
            if (typeof t9 !== "number")
              return t9.$sub();
            ...
            if (typeof t7 !== "number")
              return t7.$div();
            t6 = this.zoom;
            t5 = J.$sub$n(t2.get$y(point), actor.y);
            t4 = this.height;
            if (typeof t4 !== "number")
              return t4.$div();
            ctx.lineTo(t9 - t8 + t7 / 2 / t6, J.$add$ns(J.$add$ns(t5, t4 / 4 / this.zoom), depth));
            t4 = actor.x;
            t5 = this.width;
            t6 = this.zoom;
            t7 = actor.y;
            if (isPositiveDiagonal) {
              t8 = t3.get$x(previousPoint);
              if (typeof t8 !== "number")
                return t8.$sub();
              if (typeof t5 !== "number")
                return t5.$div();
              t7 = J.$sub$n(t3.get$y(previousPoint), t7);
              t9 = this.height;
              if (typeof t9 !== "number")
                return t9.$div();
              ctx.lineTo(t8 - t4 + t5 / 2 / t6, J.$add$ns(J.$add$ns(J.$add$ns(t7, t9 / 4 / this.zoom), depth), 47));
            } else {
              t8 = t2.get$x(point);
              if (typeof t8 !== "number")
                return t8.$sub();
            ...
            t4 = 100 - depth;
            _color = T.Camera_ColorLuminance(color, t4 / 200);
            ctx.fillStyle = _color;
            ctx.strokeStyle = _color;
            ctx.fill();
            ctx.stroke();
            ctx.beginPath();
            t5 = t3.get$x(previousPoint);
            t6 = actor.x;
            if (typeof t5 !== "number")
              return t5.$sub();
            t7 = this.width;
            if (typeof t7 !== "number")
              return t7.$div();
            t8 = this.zoom;
            t9 = J.$sub$n(t3.get$y(previousPoint), actor.y);
            t10 = this.height;
            if (typeof t10 !== "number")
              return t10.$div();
            ctx.moveTo(t5 - t6 + t7 / 2 / t8, J.$add$ns(J.$add$ns(J.$add$ns(t9, t10 / 4 / this.zoom), depth), 47));
            t5 = actor.x;
            t6 = this.width;
            t7 = this.zoom;
            t8 = actor.y;
            if (isPositiveDiagonal) {
              ...
              if (typeof t6 !== "number")
                return t6.$div();
              t8 = J.$sub$n(t2.get$y(point), t8);
              t10 = this.height;
              if (typeof t10 !== "number")
                return t10.$div();
              ctx.lineTo(t9 - t5 + t6 / 2 / t7, J.$add$ns(J.$add$ns(t8, t10 / 4 / this.zoom), depth));
            } else {
              t9 = t3.get$x(previousPoint);
              if (typeof t9 !== "number")
                return t9.$sub();
              if (typeof t6 !== "number")
                return t6.$div();
              t8 = J.$sub$n(t3.get$y(previousPoint), t8);
              t10 = this.height;
              if (typeof t10 !== "number")
                return t10.$div();
              ctx.lineTo(t9 - t5 + t6 / 2 / t7, J.$add$ns(J.$add$ns(t8, t10 / 4 / this.zoom), depth));
            ...
            if (typeof t7 !== "number")
              return t7.$div();
            t8 = this.zoom;
            t9 = J.$sub$n(t2.get$y(point), actor.y);
            t10 = this.height;
            if (typeof t10 !== "number")
              return t10.$div();
            ctx.lineTo(t5 - t6 + t7 / 2 / t8, J.$add$ns(J.$add$ns(J.$add$ns(t9, t10 / 4 / this.zoom), depth), 47));
            _color = T.Camera_ColorLuminance(color, (t4 - 23.5) / 200);
            ctx.fillStyle = _color;
            ctx.strokeStyle = _color;
            ctx.fill();
            ctx.stroke();
    ...
    static: {Camera_ColorLuminance: function(hex, lum) {
        var t1, t2, rgb, i, c;
        t1 = H.JSSyntaxRegExp_makeNative("/[^0-9a-f]/gi", false, true, false);
        hex = H.stringReplaceAllUnchecked(hex, new H.JSSyntaxRegExp(t1, null, null), "");
        if (hex.length < 6) {
          t1 = hex[0];
          t1 += t1;
          t2 = hex[1];
        ...
        for (rgb = "#", i = 0; i < 3; ++i) {
          t1 = i * 2;
          c = H.Primitives_parseInt(C.JSString_methods.substring$2(hex, t1, t1 + 2), 16, null);
          t1 = J.getInterceptor$n(c);
          t1 = C.JSNumber_methods.toInt$0(C.JSNumber_methods.roundToDouble$0(P.min(P.max(0, t1.$add(c, t1.$mul(c, lum))), 255)));
          c = t1.toString(16);
          rgb += C.JSString_methods.substring$1("00" + c, c.length);
        }
        return rgb;
      }}
  }
...
["dart.dom.html", "dart:html", , W, {
  ...
  SelectElement: {
    "": "HtmlElement;length=,selectedIndex=,value=",
...
["dart.math", "dart:math", , P, {
  ...
  Point: {
    ...
    $add: function(_, other) {
      ...
      if (typeof t3 !== "number")
        return H.iae(t3);
      return H.setRuntimeTypeInfo(new P.Point(t1 + t3, J.$add$ns(this.y, t2.get$y(other))), [H.getRuntimeTypeArgument(this, "Point", 0)]);
    },
    $sub: function(_, other) {
      var t1, t2, t3;
      t1 = this.x;
      ...
      if (typeof t3 !== "number")
        return H.iae(t3);
      return H.setRuntimeTypeInfo(new P.Point(t1 - t3, J.$sub$n(this.y, t2.get$y(other))), [H.getRuntimeTypeArgument(this, "Point", 0)]);
    },
    $mul: function(_, factor) {
      var t1 = this.x;
      if (typeof t1 !== "number")
        return t1.$mul();
      return H.setRuntimeTypeInfo(new P.Point(t1 * factor, J.$mul$n(this.y, factor)), [H.getRuntimeTypeArgument(this, "Point", 0)]);
    },
    $isPoint: true
  }
}],
...
["input", "../src/keyboard.dart", , K, {
  "": "",
  Keyboard: {
    "": "Object;_keys",
    Keyboard$0: function() {
      var t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_keydown._eventType, false), [null]);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new K.Keyboard_closure(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
      t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_keyup._eventType, false), [null]);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new K.Keyboard_closure0(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    },
    static: {Keyboard$: function() {
        var t1 = new K.Keyboard(P.HashMap_HashMap(null, null, null, J.JSInt, J.JSInt));
        t1.Keyboard$0();
        return t1;
      }}
  },
...
["stage", "../src/stage.dart", , G, {
  "": "",
  Stage_updateSubstars: function(substars, camera, actor, singleWidth) {
    var t1, t2, t3, t4, t5, previousStar, t6, t7;
    t1 = J.getInterceptor$ax(substars);
    t2 = singleWidth / 2;
    while (true) {
      t3 = J.get$y$x(t1.get$last(substars));
      t4 = actor.y;
      t5 = camera.height;
      if (typeof t5 !== "number")
        return t5.$div();
      if (!J.$gt$n(t3, t4 - t5 / 2 / camera.zoom))
        break;
      previousStar = t1.get$last(substars);
      t3 = J.getInterceptor$x(previousStar);
      t4 = t3.get$x(previousStar);
      t3 = J.$sub$n(t3.get$y(previousStar), singleWidth);
      t5 = C.C__JSRandom.nextInt$1(singleWidth);
      t6 = C.C__JSRandom.nextInt$1(singleWidth);
      t7 = C.C__JSRandom.nextInt$1(64);
      t3 = new K.Star(null, null, null, t4, t3);
      t3.shiftX = t5 - t2;
      t3.shiftY = t6 - t2;
      t3.isVisible = t7 === 0;
      t1.add$1(substars, t3);
    }
  },
  Luna: {
    "": "Stage;g,r,color,positiveSurfaceLine,negativeSurfaceLine,positiveStars,negativeStars"
  },
  Mars: {
    "": "Stage;g,r,color,positiveSurfaceLine,negativeSurfaceLine,positiveStars,negativeStars"
  },
  ...
  Stage: {
    "": "Object;",
    updateSurface$2: function(actor, camera) {
      var t1;
      this.updatePositiveSurfaceLine$3(camera, actor, 24);
      this.updateNegativeSurfaceLine$3(camera, actor, 24);
      for (t1 = this.positiveStars, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();)
        G.Stage_updateSubstars(t1._current, camera, actor, 24);
      for (t1 = this.negativeStars, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();)
        G.Stage_updateSubstars(t1._current, camera, actor, 24);
    },
    updateNegativeSurfaceLine$3: function(camera, actor, singleWidth) {
      var t1, t2, t3, t4, t5, previousPoint, newPoint, subStars, t6;
      t1 = singleWidth / 2;
      while (true) {
        t2 = C.JSArray_methods.get$last(this.negativeSurfaceLine).x;
        t3 = actor.x;
        t4 = camera.width;
        if (typeof t4 !== "number")
          return t4.$div();
        ...
        if (!(t2 > t3 - t4 / 2 / t5))
          break;
        previousPoint = C.JSArray_methods.get$last(this.negativeSurfaceLine);
        t2 = previousPoint.x;
        if (typeof t2 !== "number")
          return t2.$sub();
        t2 -= singleWidth;
        t3 = J.$add$ns(previousPoint.y, (C.C__JSRandom.nextInt$1(7) - 3) * 8);
        newPoint = new P.Point(t2, t3);
        newPoint.$builtinTypeInfo = [null];
        this.negativeSurfaceLine.push(newPoint);
        subStars = [];
        t4 = C.C__JSRandom.nextInt$1(singleWidth);
        t5 = C.C__JSRandom.nextInt$1(singleWidth);
        t6 = C.C__JSRandom.nextInt$1(3);
        t3 = new K.Star(null, null, null, t2, t3);
        t3.shiftX = t4 - t1;
        t3.shiftY = t5 - t1;
        t3.isVisible = t6 === 0;
        subStars.push(t3);
        this.negativeStars.push(subStars);
      }
    },
    updatePositiveSurfaceLine$3: function(camera, actor, singleWidth) {
      var t1, t2, t3, t4, t5, previousPoint, newPoint, subStars, t6;
      t1 = singleWidth / 2;
      while (true) {
        t2 = C.JSArray_methods.get$last(this.positiveSurfaceLine).x;
        t3 = actor.x;
        t4 = camera.width;
        if (typeof t4 !== "number")
          return t4.$div();
        ...
        if (!(t2 < t3 + t4 / 2 / t5))
          break;
        previousPoint = C.JSArray_methods.get$last(this.positiveSurfaceLine);
        t2 = previousPoint.x;
        if (typeof t2 !== "number")
          return t2.$add();
        t2 += singleWidth;
        ...
        newPoint = new P.Point(t2, t3);
        newPoint.$builtinTypeInfo = [null];
        this.positiveSurfaceLine.push(newPoint);
        subStars = [];
        t4 = C.C__JSRandom.nextInt$1(singleWidth);
        t5 = C.C__JSRandom.nextInt$1(singleWidth);
        t6 = C.C__JSRandom.nextInt$1(3);
        ...
        t3.isVisible = t6 === 0;
        subStars.push(t3);
        this.positiveStars.push(subStars);
      }
    },
    Stage$1: function(canvas) {
      var t1, t2, pointZeroRandomY;
      this.g = C.C__JSRandom.nextInt$1(10) / 10000 + 0.0005;
      this.r = C.C__JSRandom.nextInt$1(5);
      this.color = C.JSString_methods.substring$2(C.JSInt_methods.toRadixString$1(C.JSNumber_methods.toInt$0(C.JSInt_methods.roundToDouble$0(4473924 + C.C__JSRandom.nextInt$1(3355443))), 16), 1, 6);
      this.negativeSurfaceLine = [];
      t1 = J.getInterceptor$x(canvas);
      t2 = t1.get$height(canvas);
      if (typeof t2 !== "number")
        return t2.$tdiv();
      t2 = C.C__JSRandom.nextInt$1(C.JSInt_methods._tdivFast$1(t2, 4));
      t1 = t1.get$height(canvas);
      if (typeof t1 !== "number")
        return t1.$mul();
      pointZeroRandomY = t2 + C.JSInt_methods._tdivFast$1(t1 * 2, 4);
      this.negativeSurfaceLine.push(H.setRuntimeTypeInfo(new P.Point(0, pointZeroRandomY), [null]));
      t1 = [];
      this.positiveSurfaceLine = t1;
      t1.push(H.setRuntimeTypeInfo(new P.Point(0, pointZeroRandomY), [null]));
      this.positiveStars = [];
      this.negativeStars = [];
    },
    Stage$custom$4: function(g, r, color, canvas) {
      var t1, t2, pointZeroRandomY;
      this.negativeSurfaceLine = [];
      t1 = J.getInterceptor$x(canvas);
      t2 = t1.get$height(canvas);
      if (typeof t2 !== "number")
        return t2.$tdiv();
      t2 = C.C__JSRandom.nextInt$1(C.JSInt_methods.toInt$0(C.JSInt_methods._tdivFast$1(t2, 4)));
      t1 = t1.get$height(canvas);
      if (typeof t1 !== "number")
        return t1.$mul();
      pointZeroRandomY = t2 + C.JSInt_methods._tdivFast$1(t1 * 2, 4);
...
["star", "../src/star.dart", , K, {
  "": "",
  Star: {
    "": "Point;shiftX,shiftY,isVisible<,x,y",
    $asPoint: function() {
      return [null];
    }
  }
...
["", "super_lander.dart", , F, {
  "": "",
  game: [function(delta) {
    var t1, t2, t3, t4, t5, t6, t7, t8, t9, surfacePoint1, _deltaY1, altitude;
    $.stage.updateSurface$2($.actor, $.camera);
    t1 = $.actor;
    if (t1.isActive) {
      t1.toString;
      document.querySelector("#fuel").textContent = C.JSInt_methods.toString$0(C.JSNumber_methods.toInt$0(t1.fuel));
      t1 = $.actor;
      t2 = $.keyboard;
      t3 = $.stage;
      t4 = $.camera;
      t1.toString;
      t2 = t2._keys;
      if (t2.containsKey$1(65))
        t1.rotation = t1.rotation - 0.04363323129985824;
      if (t2.containsKey$1(68))
        t1.rotation = t1.rotation + 0.04363323129985824;
      if (t2.containsKey$1(87)) {
        t5 = t1.acceleration;
        if (t5 < 0.002)
          t1.acceleration = t5 + 0.0001;
        else
          t1.acceleration = 0.002;
      }
      if (t2.containsKey$1(83)) {
        t2 = t1.acceleration;
        if (t2 > 0.0003)
          t1.acceleration = t2 - 0.0001;
        else
          t1.acceleration = 0;
      }
      t2 = t1.deltaX;
      t5 = t1.rotation;
      t1.deltaX = t2 + Math.cos(t5) * t1.acceleration;
      t2 = t1.deltaY;
      t5 = t1.rotation;
      t5 = Math.sin(t5);
      t6 = t1.acceleration;
      ...
      if (typeof t8 !== "number")
        return t8.$div();
      t9 = t8 / 3 / t9;
      if (t7 - t2 > t9 || t2 - t7 > t9)
        t4.y = t7 + t5;
      if (t6 > 0) {
        t2 = t3.positiveSurfaceLine;
        t6 = C.JSNumber_methods._tdivFast$1(t6, 24);
        t3 = t2.length;
        if (t6 >>> 0 !== t6 || t6 >= t3)
        ...
        _deltaY1 = t5.$sub(t3, t2[t6].y);
        t6 = t1.x;
        surfacePoint1 = surfacePoint1.x;
        if (typeof surfacePoint1 !== "number")
          return H.iae(surfacePoint1);
        if (typeof _deltaY1 !== "number")
          return H.iae(_deltaY1);
        altitude = J.$sub$n(J.$sub$n(t5.$sub(t3, (t6 - surfacePoint1) * _deltaY1 / 24), t1.y), 8);
      } else {
        t2 = t3.negativeSurfaceLine;
        t6 = C.JSNumber_methods._tdivFast$1(-t6, 24);
        t3 = t2.length;
        if (t6 >>> 0 !== t6 || t6 >= t3)
          return H.ioore(t2, t6);
        ...
        if (typeof _deltaY1 !== "number")
          return H.iae(_deltaY1);
        altitude = J.$sub$n(J.$sub$n(J.$add$ns(t3, (t6 - surfacePoint1) * _deltaY1 / 24), t1.y), 8);
      }
      t2 = J.getInterceptor$n(altitude);
      if (t2.$lt(altitude, 1)) {
        t3 = t1.deltaX;
        if (t3 < 0.2 && t3 > -0.2 && t1.deltaY < 0.2 && true)
          J.set$display$x(document.querySelector("#congratulation").style, "block");
        else
          J.set$display$x(document.querySelector("#commiseration").style, "block");
        t1.isActive = false;
        t1.acceleration = 0;
      }
      t1 = t4.height;
      if (typeof t1 !== "number")
        return t1.$div();
      if (t2.$lt(altitude, t1 / 2 / t4.zoom)) {
        t1 = t4.zoom;
        if (t1 < 4)
          t4.zoom = t1 + 0.0015;
      } else {
      ...
      document.querySelector("#altitude").textContent = C.JSInt_methods.toString$0(t2.toInt$0(altitude));
    }
    t1 = $.camera;
    t2 = $.get$ctx();
    t3 = $.stage;
    ...
    t5 = t1.zoom;
    t2.scale(t5, t5);
    t1.drawStars$3(t2, t3.positiveStars, t4);
    t1.drawStars$3(t2, t3.negativeStars, t4);
    t1.drawSurfaceLine$4(t2, t3.positiveSurfaceLine, t3.color, t4);
    t1.drawSurfaceLine$4(t2, t3.negativeSurfaceLine, t3.color, t4);
    t2.beginPath();
    t2.save();
    t2.fillStyle = "#aaa";
    t2.strokeStyle = "#aaa";
    ...
    if (typeof t6 !== "number")
      return t6.$div();
    t2.translate(t5 / 2 / t3, t6 / 4 / t3);
    t2.rotate(t4.rotation);
    t2.moveTo(6, 0);
    t2.lineTo(4, -4);
    t2.lineTo(4, 4);
    t2.fillStyle = "#bbb";
    ...
    t2.moveTo(-6, -3);
    t4 = t4.acceleration;
    t2.lineTo(-t4 * 24 * 500 - 6 + C.C__JSRandom.nextInt$1(3), 0);
    t2.lineTo(-6, 3);
    t2.closePath();
    t2.fill();
    t2.beginPath();
    ...
    t1 = 1 / t1.zoom;
    t2.scale(t1, t1);
    $.animationFrame = C.Window_methods.get$animationFrame(window).then$1(F.game$closure());
  }, "call$1", "game$closure", 2, 0, 9],
  init: [function(e) {
    J.set$display$x(document.querySelector("#menu-wrap").style, "none");
    J.set$display$x(document.querySelector("#statistics").style, "block");
    $.actor.isActive = true;
  }, "call$1", "init$closure", 2, 0, 10],
  selectChangeHandler: [function(e) {
    var select, t1;
    select = document.querySelector("#stage");
    t1 = J.getInterceptor$x(select);
    switch (J.get$value$x(J.$index$asx(t1.get$options(select), t1.get$selectedIndex(select)))) {
      case "luna":
        t1 = new G.Luna(0.001, 0, "707060", null, null, null, null);
        t1.Stage$custom$4(0.001, 0, "707060", $.get$canvas());
        $.stage = t1;
        break;
      case "mars":
        t1 = new G.Mars(0.0013, 0.2, "E3701A", null, null, null, null);
        t1.Stage$custom$4(0.0013, 0.2, "E3701A", $.get$canvas());
        $.stage = t1;
        break;
      case "random":
        t1 = new G.RandomStage(null, null, null, null, null, null, null);
        t1.Stage$1($.get$canvas());
        $.stage = t1;
        break;
      default:
    }
  }, "call$1", "selectChangeHandler$closure", 2, 0, 11],
  main: [function() {
    var t1, t2, e;
    t1 = $.get$canvas();
    J.set$width$x(t1, window.innerWidth);
    J.set$height$x(t1, window.innerHeight);
    $.keyboard = K.Keyboard$();
    t2 = new E.Actor(0, 0, null, null, null, null, null, null, null, null, null, null);
    t2.Actor$2(0, 0);
    $.actor = t2;
    t1 = new T.Camera(null, null, J.get$width$x(t1), J.get$height$x(t1), 1);
    t1.x = 0;
    t1.y = 0;
    $.camera = t1;
    e = document.createEvent("Event");
    e.initEvent("ds", true, true);
    F.selectChangeHandler(e);
    t1 = document.querySelector("#play");
    t1.toString;
    t2 = C.EventStreamProvider_click._eventType;
    t1 = H.setRuntimeTypeInfo(new W._ElementEventStreamImpl(t1, t2, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(F.init$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    t1 = document.querySelector("#stage");
    t1.toString;
    t1 = H.setRuntimeTypeInfo(new W._ElementEventStreamImpl(t1, C.EventStreamProvider_change._eventType, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(F.selectChangeHandler$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    t1 = document.querySelector("#win-reload");
    t1.toString;
    t1 = H.setRuntimeTypeInfo(new W._ElementEventStreamImpl(t1, t2, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(F.reloadHandler$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    t1 = document.querySelector("#fail-reload");
    t1.toString;
    t2 = H.setRuntimeTypeInfo(new W._ElementEventStreamImpl(t1, t2, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(F.reloadHandler$closure()), t2._useCapture), [H.getTypeArgumentByIndex(t2, 0)])._tryResume$0();
    $.animationFrame = C.Window_methods.get$animationFrame(window).then$1(F.game$closure());
  ...
  }, "call$1", "reloadHandler$closure", 2, 0, 11]
},
1],
]);
...
Isolate.$lazy($, "ctx", "ctx", "get$ctx", function() {
  var t1 = H.interceptedTypeCast($.get$canvas(), "$isCanvasElement");
  t1.toString;
...
function dart_precompiled($collectedClasses) {
  ...
  function Actor(x, y, deltaX, deltaY, deltaVx, deltaVy, acceleration, rotation, fuel, time, isActive, isCrashed) {
    this.x = x;
    this.y = y;
    this.deltaX = deltaX;
    this.deltaY = deltaY;
  ...
  Actor.builtin$cls = "Actor";
  if (!"name" in Actor)
    Actor.name = "Actor";
  $desc = $collectedClasses.Actor;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Actor.prototype = $desc;
  Actor.prototype.get$x = function(receiver) {
    return this.x;
  };
  Actor.prototype.get$y = function(receiver) {
    return this.y;
  };
  function Camera(x, y, width, height, zoom) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
  ...
  function Luna(g, r, color, positiveSurfaceLine, negativeSurfaceLine, positiveStars, negativeStars) {
    this.g = g;
    this.r = r;
    this.color = color;
    this.positiveSurfaceLine = positiveSurfaceLine;
    this.negativeSurfaceLine = negativeSurfaceLine;
    this.positiveStars = positiveStars;
    this.negativeStars = negativeStars;
  }
  Luna.builtin$cls = "Luna";
  if (!"name" in Luna)
    Luna.name = "Luna";
  $desc = $collectedClasses.Luna;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Luna.prototype = $desc;
  function Mars(g, r, color, positiveSurfaceLine, negativeSurfaceLine, positiveStars, negativeStars) {
    this.g = g;
    this.r = r;
    this.color = color;
    this.positiveSurfaceLine = positiveSurfaceLine;
  ...
  Mars.builtin$cls = "Mars";
  if (!"name" in Mars)
    Mars.name = "Mars";
  $desc = $collectedClasses.Mars;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Mars.prototype = $desc;
  function RandomStage(g, r, color, positiveSurfaceLine, negativeSurfaceLine, positiveStars, negativeStars) {
    this.g = g;
    this.r = r;
    this.color = color;
    this.positiveSurfaceLine = positiveSurfaceLine;
  ...
  RandomStage.builtin$cls = "RandomStage";
  if (!"name" in RandomStage)
    RandomStage.name = "RandomStage";
  $desc = $collectedClasses.RandomStage;
  if ($desc instanceof Array)
    $desc = $desc[1];
  RandomStage.prototype = $desc;
  function Stage() {
  }
  ...
  function Star(shiftX, shiftY, isVisible, x, y) {
    this.shiftX = shiftX;
    this.shiftY = shiftY;
